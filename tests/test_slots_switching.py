...
def test_start_xray_uses_candidate_or_last_good_config(m, manager_factory, candidate_factory):
    ...
    calls = []
    instance.start_initial_candidate = lambda item, reason: calls.append((item.id, reason))
    instance.start_xray()
    assert calls == [(candidate.id, "Xray start")]
...
def test_blue_green_switch_success_path(m, manager_factory, candidate_factory, monkeypatch):
    ...
    instance.switch_candidate_blue_green(new, "manual")
