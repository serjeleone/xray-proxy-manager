...
При `latency_test_parallelism: 0` приложение автоматически выбирает безопасное число одновременных проверок — не более восьми и не больше числа outbound. Значение от `1` до `32` задаёт явный предел. Следующий период автоматической проверки начинается после завершения всей проверки и возможного одного переключения.
